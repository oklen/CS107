...
template<class T>
void PutOutProduction(const T& production)
{
  auto randomProduction =  grammar[production].getRandomProduction();
  auto ProductionIt = randomProduction.begin();
  while(ProductionIt != randomProduction.end())
    {
      if((*ProductionIt).at(0) != '<')
        {
          cout << *ProductionIt;
          if(ProductionIt < randomProduction.end()-1)
            cout << " ";
        }
      else
        {
          PutOutProduction<T>(*ProductionIt);
        }
      ProductionIt++;
    }
}
...
int main(int argc, char *argv[])
{
  if (argc == 1) {
    ...
    return 1; // non-zero return vaue means something bad happened
  }
  ...
  while(it != grammar.end())
    {
      cout << (*it).first << ":" ;
      PutOutProduction<string>((*it).first);
      cout << endl;
      it++;
    }
